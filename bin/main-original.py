...
from core import FFmpegWorker, FreeFactoryCore, FFmpegWorkerZone, StreamWorker
...
class FreeFactoryApp(QMainWindow):
    def __init__(self):
        ...
        self.factory_dirty = False  # This is for when changing factories without saving first. NOT YET USED!
#=======add Stream_Worker Class
        #self.current_stream_worker = None
        self.active_streams = [] # maps rtmp_url to StreamWorker

#=======Populate Streaming Factory list
        factory_dir = self.config.get("FactoryLocation") or "/opt/FreeFactory/Factories"
        factory_names = sorted(Path(factory_dir).glob("*"))

        factory_names = [f.stem for f in factory_names if f.is_file()]
        self.streamFactorySelect.addItems(factory_names)

#=======Streaming Start and Stop buttons
        #self.StartStream.clicked.connect(lambda: start_single_stream(self))
        #self.StopStream.clicked.connect(lambda: stop_single_stream(self))
...
#=======Populate DefaultFactoryGlobal ComboBox
        ...
        self.StartStream.clicked.connect(lambda: print(self.build_streaming_command()))
...
#=======Streaming Start/Stop button ConnectionResetError
...
#===Add files to encode queue and button controls
...
#===Populate Factory List
    ...
    #=======Populate QLineEdits (excluding FactoryFilename)
        for key in [
            ...
            "VideoStreamID", "AudioStreamID", "EncodeLength", "streamInputVideo",
            "streamInputAudio", "streamRTMPUrl", "streamKey"
        ]:
...
#=======Custom key-to-field mapping for mismatched keys
        ...
        for widget_key, factory_key in lineedit_key_map.items():
            ...
            if widget:
                value = factory_data.get(factory_key, "")
...
#===Streaming command
    def build_streaming_command(self):
        ...
        # Load factory
        factory_location = self.config.get("FactoryLocation")
        factory_path = Path(factory_location) / factory_name
        factory = self.core.load_factory(factory_path)
        if not factory:
        ...
        if manual_contains_input:
            ...
            # Default path using GUI + factory input settings
            video_format = self.ForceFormatInputVideo.currentText().strip()
            video_input = self.streamInputVideo.text().strip()
            audio_format = self.ForceFormatInputAudio.currentText().strip()
            audio_input = self.streamInputAudio.text().strip()
            ...
            #pix_fmt = factory.get("PIX_FMT", "").strip()  # TODO: future support
            ...
            # Extra manual options (filters, metadata, etc.)
            if manual:
                cmd += shlex.split(manual)
...
#===New Factory (Clears all widgets except factorylist and globals)
    def new_factory(self):
        ...
        for key in [
            "FFMxProgram", "VideoCodec", "SubtitleCodecs", "VideoWrapper", "VideoFrameRate", "VideoSize",
            "VideoBitrate", "VideoAspect", "VideoTarget", "VideoTags", "AudioCodec",
            "AudioBitrate", "AudioSampleRate", "AudioExtention", "AudioChannels",
            "FTPProgram", "Threads", "VideoPreset", "VideoPixFormat"
        ]:
            widget = getattr(self, key, None)
            if widget:
...
#=======Determine output extension from current combo boxes
        ...
        print("DEBUG preview ForceFormat:", repr(self.ForceFormat.currentText().strip()))
#=======Simulate a "factory" dictionary from current form values
        factory = {
            ...
            "VIDEOPIXFORMAT": self.VideoPixFormat.currentText().strip(),
            "ENCODELENGTH": self.EncodeLength.text().strip(),
            
            "VIDEOPIXFORMAT": self.VideoPixFormat.currentText().strip(),
            
            "AUDIOSTREAMID": self.AudioStreamID.text().strip(),
            "FORCEFORMAT": self.ForceFormat.currentText().strip(),
            "MANUALOPTIONS": self.ManualOptions.text().strip(),
        }
    #=======Generate command using current GUI settings
        cmd_list = self.core.build_ffmpeg_command(input_file, factory, preview=True)
