...
def parse_muxers(output: str, conn: sqlite3.Connection, update_only=False):
    cursor = conn.cursor()
    if not update_only:
        cursor.execute("DROP TABLE IF EXISTS muxers")
        cursor.execute("CREATE TABLE muxers (name TEXT PRIMARY KEY, description TEXT)")

    muxer_line_pattern = re.compile(r"^\s*([ D][ E][a-z]?)\s+(\S+)\s+(.*)$")

    for line in output.splitlines():
        match = muxer_line_pattern.match(line)
        if match:
            flags, name, desc = match.groups()
